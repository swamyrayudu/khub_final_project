...
export default function LeafletMap({
  ...
  useEffect(() => {
    ...
    // Restore scroll position after map init
  ...
  useEffect(() => {
    ...
    mapRef.current.setView([userLocation.lat, userLocation.lng], 13, {
      animate: false, // Disable animation to prevent scroll
    });
    ...
    stores.forEach((store) => {
      ...
      const popupContent = `
        <div style="min-width: 200px;">
          ...
          <p style="margin: 0 0 4px; color: #dc2626; font-size: 11px; font-weight: 500; text-transform: uppercase;">${store.shopName || 'Store'}</p>
          <h3 style="margin: 0 0 4px; font-weight: 600; font-size: 14px;">${store.productName || store.name}</h3>
      ...
      // Add tooltip with shop name
      marker.bindTooltip(store.shopName || store.name, {
        permanent: false,
        direction: 'top',
        offset: [0, -40],
  ...
  useEffect(() => {
    if (!mapRef.current || !isRoutingLoaded || !showDirections || !userLocation) return;
    ...
    routingControlRef.current = L.Routing.control({
      ...
      altLineOptions: {
        styles: [{ color: '#9ca3af', weight: 4, opacity: 0.5 }],
        extendToWaypoints: true,
        missingRouteTolerance: 0,
      },
    }).addTo(mapRef.current);
